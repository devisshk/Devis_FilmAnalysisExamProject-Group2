...
#Worst 5 Genre by Audience
```{r}
worst_genre_aud <- dbGetQuery(con, "
SELECT films."genre(s)"" ", AVG(rotten_tomatoes_scores.audience_score) AS avg_audscore
FROM rotten_tomatoes_scores
JOIN films ON films.film_id = rotten_tomatoes_scores.film_id
GROUP BY films."genre(s)""
ORDER BY avg_audscore
...
ggplot(worst_genre_aud, aes(x = genre, y = avg_audscore)) +
  geom_bar(stat = "identity", fill = "red") +
  labs(x = "Genre", y = "Average Audience Score") +
  ggtitle("Worst Genres rated by Audience") +
  theme_minimal()
